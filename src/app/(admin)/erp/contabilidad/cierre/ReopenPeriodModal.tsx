...
import { Lock, Unlock, AlertTriangle, X, CheckCircle2, Loader2 } from "lucide-react";
import { reopenMonthlyPeriodAction } from "./actions";
...
export default function ReopenPeriodModal({ period, canEdit }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await reopenMonthlyPeriodAction(formData);
      setIsOpen(false);
      setReason("");
    } catch (err: any) {
      setError(err.message || "Error al reabrir el período.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 text-xs font-semibold bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-300 px-3 py-1.5 rounded-xl transition-colors shadow-sm"
      >
        <Unlock size={14} /> Reabrir Período para Corrección
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-lilac-100 relative">
            ...
            <div className="flex items-center gap-2.5 mb-2 text-amber-700 font-bold text-base">
              ...
              <span>Reapertura de Período Contable</span>
            </div>

            <p className="text-xs text-ink-600 mb-4">
              Estás a punto de reabrir el período <strong>{period}</strong>. Esto anulará el asiento de cierre previo para permitir ajustes en transacciones.
            </p>

            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4 text-xs text-amber-800 space-y-1">
              <p className="font-semibold">⚠️ Trazabilidad y Rastro de Auditoría:</p>
              <p>Esta acción y su motivo quedarán registrados en el historial de auditoría del sistema con tu usuario y fecha actual.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ej. Registro tardío de factura de proveedor #104 o corrección de retención de IVA..."
