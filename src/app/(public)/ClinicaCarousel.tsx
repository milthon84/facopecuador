...
export default function ClinicaCarousel({ posts = [] }: Props) {
  ...
  return (
    <div className="relative w-full h-[520px] sm:h-[580px] select-none group/carousel">
      ...
      {/* ── TARJETA SECUNDARIA SUPERPUESTA — Esquina inferior derecha (Al clic avanza al siguiente) ── */}
      {totalItems > 1 && (
        <div
          onClick={next}
          title="Ver siguiente afiche"
          className={`absolute bottom-0 right-0 w-56 sm:w-64 h-72 sm:h-80 rounded-2xl overflow-hidden shadow-2xl border-2 border-white/90 bg-slate-900 cursor-pointer
      ...
      <div
        ...
        className={`absolute top-0 left-0 rounded-3xl overflow-hidden shadow-2xl shadow-purple-950/15 bg-slate-950 border border-slate-700/60 cursor-pointer
